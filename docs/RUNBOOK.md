...
- If the API is down, the run stops early with a warning and keeps what it has. A wrong key or no credit fails loudly: `anthropic.BadRequestError: ... "Your credit balance is too low to access the Anthropic API. Please go to Plans & Billing to upgrade or purchase credits."` — this happened mid-run on 2026-09-19 (docs/DECISIONS.md), independent of however much of any self-imposed spending budget had been tracked; the two aren't the same thing, and the real balance isn't visible from inside a session. Add credit at https://platform.claude.com (Plans & Billing) before retrying; nothing already committed is at risk, and whatever `make golden-live`/`make normalise`/`make content` step was interrupted just needs re-running (cached answers aren't re-paid for).
...
Batches 01–08 have been drafted; 01–08 are approved and made live (2026-09-17/18/19) — see `docs/DECISIONS.md` for what each batch contains and every fix an approval made. `config/drafts/` holds only what's still pending: any compound explicitly **held back** from an approved batch (a UK legality question, or — for silica's `k46`/`k47` — two labels waiting on an API-credit top-up before they can be live-validated) stays there as its own small file with a note explaining why, never silently dropped. As of 2026-09-19 that's `batch_05.yml` (kanna), `batch_06.yml` (agmatine, plus the pending silica labels), `batch_07.yml` (dmae, uridine), `batch_08.yml` (nmn, lithium orotate) — see docs/DECISIONS.md for the exact question against each.

Label ids must not collide with the live set: `g` = prototype, `d/e/f/h` = batches 1–4, `j` = batch 5, `k` = batch 6, `m` = batch 7, `n` = batch 8 (also used, harmlessly, by the unrelated `negatives:` fixtures — a different top-level key the golden harness never reads together with `labels:`). Use a fresh letter for the next batch (skip `l`, easily confused with `1`).

**Independent audit (used for delegated/unattended runs, e.g. 2026-09-19):** when nobody is available to review a batch personally, a separate subagent that did not draft it can do the checklist above instead — Appendix F match, elemental factors recomputed independently, name collisions checked against the *whole* live registry (not just the new batch), test-label realism, compliance/language, UK legal status — and give a PASS/FIX/HOLD verdict per compound. Fix every FIX, re-run `make golden`, and have the auditor re-check (two rounds at most); anything still unresolved gets held out of the batch, not guessed at. This caught real issues a solo pass missed every time it was used: two live-registry `accepted_cofactors` gaps and an alias collision in batch 5; a live alias_exclusions gap that had a genuine effect on a real reading in batch 6, plus two more real collisions and a design flaw in how ambiguous readings were handled; a missing live-registry fix in batch 7; a real medicine-name collision in batch 8. It is not a substitute for you looking at a batch when you have time to — it's what runs when you don't.

To approve a batch: tell Claude "approve batch NN" (or by hand: move the compounds into `config/compounds.yml`, the formulas into `config/factor_sources.yml`, the labels into `tests/golden/labels.yml` — matching its established 2-space list-item indent, not the draft files' 0-indent — and the saved readings into `tests/golden/cache/`, then `make check`; hold back anything with an open question by leaving it in the draft file with a note). New supplements only get pages once a retailer's data contains products for them.
