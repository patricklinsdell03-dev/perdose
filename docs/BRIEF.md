...
Version 1.9 · 2026-09-18 · Owner: Patrick · Builder: Claude Code
...
**Changelog v1.8 (2026-09-18):** Patrick's two corrections. (1) **Completeness is the point**: the target is every reputable UK seller of each product; a retailer that refuses affiliate access is named on the methodology page as not listed, never silently omitted, and reapplied to on a schedule. (2) **Amazon is a core source, not a v1.1 extra**: it enters every table at the Buy Box price through the Product Advertising API (refreshed daily per Amazon's terms) as soon as the Associates account qualifies (3 sales in 180 days); until then affiliate search links on product pages earn those sales. Marketplace listings are held to the same unverified-if-ambiguous rule, and a "reliable and reputable" line for third-party sellers is a pending decision. §19 item 4 is superseded. **Basket builder (Patrick, same day; replaces §19 item 8's scope):** pick several supplements and doses, and the site finds the cheapest way to buy them with two filters — *all from one retailer* and *delivered within N days* (each retailer's standard delivery time and cost live in `config/retailers.yml`; delivery cost counts, so a one-retailer basket that clears the free-delivery threshold can win). Client-side only, shareable by URL, no accounts. Built once several live feeds exist.

**Changelog v1.9 (2026-09-18, Patrick):** (1) **The home page is the control surface**: search as the hero plus a quick-compare strip (supplement → form → daily dose → retailers → include-abroad) that lands on the table already filtered; the table filters are all URL state so home can set them. (2) **"Build a basket" is its own tab** (`/basket/`): the user adds supplements and daily doses, sets *order from at most X websites*, and the site finds the cheapest split across ≤ X retailers including each retailer's delivery charge, shown as one sub-basket per retailer with a link per product (Amazon: one add-to-cart link for the whole sub-basket where its API allows; other retailers: one link per product). Client-side only, shareable by URL, no accounts. Built now on the seed data so it is ready when feeds arrive. **Combinations section (same day):** multivitamins and multi-ingredient products in their own section at `/combinations/`, filtered by minimum daily amounts per ingredient with named presets (ZMA, D3+K2, …), sorted by price per daily serving; multivitamins are compared `per_serving`.

**Changelog v1.10 (2026-09-19, Patrick):** the site's look brought up to the Capsule mock-up B2 he chose, desktop and home page first. §12.4 rule 3 now describes card rows at every width (still a table underneath) instead of a plain table on wide screens; the refinement list is updated. Detail in DECISIONS.md.
...
3. The per-dose price is always the largest figure in a row. Every product is a card (mock-up B2): rank capsule; the name with a meta line (brand · retailer · pack · per serving); three figure tiles (per dose — tinted and biggest — per month, lasts); the buy capsule; the working. Phones stack the card; wider screens put the product on the left and the tiles and capsule on the right, in the same place on every card, so the figures scan down the page side by side. Underneath it stays a table (with explicit roles) for screen readers and the sort/filter script. The cheapest row carries a thin teal ring and shows its working open. *(v1.10; was: a table on wide screens.)*
...
**Known refinement list (to do at the polish pass):** the basket page needs a full design pass within the Capsule language (it has the new controls and tiles, not a redesign); a final pass on spacing with Patrick looking at real phones. *Done in the refinement pass of 2026-09-19 (v1.10, DECISIONS.md):* price rows as B2 cards at every width with the cheapest ringed in teal and its working open; a slim sticky dose bar with sort and filters folded into one panel; price-page headings with a "what it really costs" line and the headline figure; a designed empty state for forms found only in combination products; the home page with a real example card, a one-line quick compare and cards grouped by ingredient class (with whose price it is and how much was compared); header and footer; product and browse pages; dark-mode contrast for teal and the rank numbers. *Done since v1.5 (see DECISIONS.md):* home page opening with search as the hero; product names split into name + pack size; claimed chips stay in the row (quiet grey) because shoppers choose on them; share cards drawn in Red Hat Display; search results rebuilt as our own list (comparisons first, then products, each with one factual line); long-form pages given a reading column, ruled headings and the sums set in the figures typeface.
